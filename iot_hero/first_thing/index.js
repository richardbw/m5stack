...
const topic = `herogroup/${thingName}`
//was: const topic = `heroiotworkshop/${thingName}`
...
// Replace the device.on('connect', () => {...}) with: 
device.on('connect', () => {
    ...
    device.publish(topic, JSON.stringify({ message: " pi2> I am an AWS IoT Hero :-)" }))
...
// We are listening to incoming messages
